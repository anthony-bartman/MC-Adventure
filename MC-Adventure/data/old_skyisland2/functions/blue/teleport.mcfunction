...
execute at @e[tag=blueTeamCen] run gamemode survival @a[team=blueTeam,scores={survivalOn=1},distance=..75]
execute at @e[tag=blueTeamCen] run gamemode adventure @a[team=!blueTeam,distance=..75,scores={survivalOn=0..1}]

#TP from MAIN TO ISLAND
#------
#From Main Island to Island
execute at @e[tag=blueTeamMainTp] if entity @a[team=blueTeam,distance=..8] run particle minecraft:happy_villager ~ ~-1 ~ .1 .1 .1 .1 1 force
execute at @e[tag=blueTeamMainTp] if entity @a[team=blueTeam,distance=..8] run particle minecraft:end_rod ~ ~1 ~ .01 .01 .01 .1 1 force
execute at @e[tag=blueTeamMainTp] as @a[team=blueTeam,distance=..1.5] run title @s title {"text":"Welcome Home"}
execute at @e[tag=blueTeamMainTp] as @a[team=blueTeam,distance=..1.5] run title @s subtitle {"selector":"@s"}
execute at @e[tag=blueTeamMainTp] as @a[team=blueTeam,distance=..1.5] run effect give @s minecraft:blindness 3 10 true
execute at @e[tag=blueTeamMainTp] as @a[team=blueTeam,distance=..1.5] run effect give @s minecraft:slow_falling 8 1 true
execute at @e[tag=blueTeamMainTp] as @a[team=blueTeam,distance=..1.5] at @s run playsound minecraft:entity.enderman.teleport master @s ~ ~ ~ 100
#Makes Tp Altar unbreakable
execute at @e[tag=blueTeamMainTp] if entity @a[distance=..10] run setblock ~ ~-1 ~ minecraft:sea_lantern
execute at @e[tag=blueTeamMainTp] if entity @a[distance=..10] run kill @e[type=item,distance=..1.5]
execute at @e[tag=blueTeamMainTp] as @a[team=blueTeam,distance=..1.5] run teleport @s @e[tag=blueTeamh,limit=1]
