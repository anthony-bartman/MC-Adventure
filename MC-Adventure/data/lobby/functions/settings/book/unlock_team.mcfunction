...
execute if score lockTeams lobbySettings matches 1 run scoreboard players set lockTeams lobbySettings 0
#Kills armor stands that say LOCKED
#Summon Armor stand saying its locked
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=goldLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=purpleLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=greenLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=aquaLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=redLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=yellowLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=blueLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=blackLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=cyanLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=magentaLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=silverLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=crimsonLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=cobaltLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=ivyLock]
execute if score lockTeams lobbySettings matches 0 run kill @e[tag=opalLock]
...
execute at @e[tag=opalTeam] run summon minecraft:armor_stand ~ ~-0.5 ~ {Tags:["opalTeamj"],CustomNameVisible:1b,CustomName:'{"text":"JOIN","color":"dark_green","bold":true}',NoGravity:1,Invisible:1,DisabledSlots:2039583,Marker:1}
