...
execute if score @s chooseKit matches 1.. run title @s title ["",{"text":"Welcome ","bold":true,"color":"aqua"},{"selector":"@s[team=yellowTeam]","italic":true}]
execute if score @s chooseKit matches 1.. run teleport @s @e[type=minecraft:armor_stand,tag=yellowIntro,limit=1]
