...
execute if score Teammates cyTeamPlayers >= cyanTeam maxPlayers run title @s subtitle [{"text":"Already Contains ","bold":true},{"score":{"name":"cyanTeam","objective":"maxPlayers"},"color":"dark_red","bold":true},{"text":" Players","color":"white"}]
...
execute if score Teammates cyTeamPlayers < cyanTeam maxPlayers if score kitsEnabled lobbySettings matches 1 run title @s actionbar {"text":"Select a Kit","color":"yellow","bold":true}
#Sets 'Kit' score to 0 (No Kit)
execute if score Teammates cyTeamPlayers < cyanTeam maxPlayers if score kitsEnabled lobbySettings matches 1 run scoreboard players enable @s chooseKit
execute if score Teammates cyTeamPlayers < cyanTeam maxPlayers if score kitsEnabled lobbySettings matches 1 as @s run function lobby:kits/selection_book
