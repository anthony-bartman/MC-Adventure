...
execute if score Teammates crTeamPlayers >= crimsonTeam maxPlayers run title @a subtitle [{"text":"Already Contains ","bold":true},{"score":{"name":"crimsonTeam","objective":"maxPlayers"},"color":"dark_red","bold":true},{"text":" Players","color":"white"}]
...
execute if score Teammates crTeamPlayers < crimsonTeam maxPlayers run title @s actionbar {"text":"Select a Kit","color":"yellow","bold":true}
#Sets 'Kit' score to 0 (No Kit)
execute if score Teammates crTeamPlayers < crimsonTeam maxPlayers run scoreboard players enable @s chooseKit
execute if score Teammates crTeamPlayers < crimsonTeam maxPlayers as @s run function lobby:kits/selection_book
