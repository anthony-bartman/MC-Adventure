...
execute if score kitsEnabled lobbySettings matches 0 run title @s title ["",{"text":"Welcome ","bold":true,"color":"aqua"},{"selector":"@s[team=spectators]","italic":true}]
execute if score kitsEnabled lobbySettings matches 0 run teleport @s @e[type=minecraft:armor_stand,tag=spectatorsIntro,limit=1]
...
execute if score kitsEnabled lobbySettings matches 1 if score @s chooseKit matches 1.. run teleport @s @e[type=minecraft:armor_stand,tag=spectatorsIntro,limit=1]
