...
execute at @e[type=minecraft:armor_stand,tag=blackTeami,limit=1] if entity @s[distance=..5] as @e[type=item,tag=,distance=..3] run function skyisland:adv/inventory

#-- Make everyone teleport back home during adventure
execute at @e[type=minecraft:armor_stand,tag=blackTeamCen,limit=1] as @a[team=!blackTeam,tag=player,distance=..60] run function skyisland:adv/tp_homeskyisland
#Volcano TP
...
#Raid Setup
execute if score blackMP mapProgress matches 13 if score loopSpeed lobbySettings matches 0 if score blackTime raidControl matches 0.. run function skyisland:teams/black/adv/raid/setup0
execute if score blackMP mapProgress matches 13 if score loopSpeed lobbySettings matches 1 if score blackTime raidControl matches 0.. run function skyisland:teams/black/adv/raid/setup1
execute if score blackMP mapProgress matches 13 if score loopSpeed lobbySettings matches 2 if score blackTime raidControl matches 0.. run function skyisland:teams/black/adv/raid/setup2
execute if score blackMP mapProgress matches 13 if score loopSpeed lobbySettings matches 3 if score blackTime raidControl matches 0.. run function skyisland:teams/black/adv/raid/setup3
...
execute if score blackMP mapProgress matches 12..29 at @s as @s[y=0,dy=100] run function skyisland:teams/black/tp_safe

#Stage 6 TP altar (Progress Nums: 28-30)
#--Checks if altar is setup correctly (20)
execute if score blackMP mapProgress matches 30.. as @e[type=minecraft:armor_stand,tag=blackTeama,limit=1] at @s if entity @a[tag=player,distance=..12] run function skyisland:teams/black/adv/teleport_main
